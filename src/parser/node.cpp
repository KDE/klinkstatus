...
void NodeMETA::parseAttributeURL()
{
    ...
    if(upperCase(attribute_http_equiv_) == "REFRESH")
    {
        ...
        if(attribute_url_.isEmpty())
            cerr << "void NodeMeta::parseAttributeURL(): Assertion `!attribute_url_.isEmpty()' failed.\n"
...
void NodeIMG::parseAttributeSRC()
{
    ...
    {
        cerr << "MALFORMED_____________________________________________________________" << endl;
        cerr << "Conteudo: " << content() << endl;
        setMalformed(true);
        return;
    }
